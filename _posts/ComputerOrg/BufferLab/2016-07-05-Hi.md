...
From above Assembly code we can see that register %eax is fully occupied by the content of memory address 0x804d104. Then the content of register %eax is compared with the content of memory address 0x804d10c. The result of comparison must be equal so that the program can execute the next Assembly code normally. Then what's the content of address 0x804d104 and 0x804d10c?
...
Evidently, the content of address 0x804d104 is the value of global variable **Hi** which is now worth 0. Afterwards, the content of address 0x804d10c is our cookie which is now worth 0. Why is the value 0? For we haven't execute the bufbomb program. Ok, so we have gotten these important information:

> * Address of procedure **annyeong()**: 0x08048bf3.<br />
> * The content of address 0x804d104 equals to the content of global variable Hi (0) which is moved into register %eax.<br />
...
After filling register %eax with the content of global variable **Hi**, the next step is to check whether the content of register %eax is the same with our cookie. If not, the program will issue an error message. The conclusion is the content of register %eax must be same with our cookie. So how do we change the content of register %eax? Simply put, we can only just change the content of address 0x804d104 into our cookie (0x1d9ed824).

Afterwards, the program will re-check **cmpl $0xffffffff, 0x804d100** whether the content of address 0x804d100 is equal to a constant 0xffffffff or not. If not, the program will issue an error message. So how do we change it so that the value is equal to the constant? The step is similar to the previous explanation, namely we only change the content of 0x804d100.

Ok, now we'll simplify what we need to do on this level:

> 1. Change the content of address 0x804d104 into our cookie (0x1d9ed824).<br />
> 2. Change the content of address 0x804d100 into a constant 0xffffffff.<br />

To change the content of an address, we can't just directly do the thing through user input. As both of addresses aren't located within the stack, we need to inject codes where the content is also Assembly code. 
...
The first row of the above code shows us that it'll change the content of address 0x804d104 into a constant 0x1d9ed824 which is our cookie value.

The second row of the above code shows us that it'll change the content of address 0x804d100 into a constant 0xffffffff.
...
Why do we fail? It's fail as from out string input we gave the return address of procedure **annyeong()** after the free 52 characters. The effect is after the program was finished in reading that 52 characters, it won't execute our code injection, yet directly return to procedure **annyeong()**. So, the solution is we've to change the content of block after our 52 characters so that the program can read our code injection. Need to remember that from our code injection we had already given a command that after changing the content of 2 memory addresses, we should directly insert he address of procedure **annyeong()** which will make the program redirects the flow into procedure **annyeong()** after the function **ah_choo()** was finished.
...
Then look at the content of function **ah_choo()**.
...
After we give 4 **ni** instructions, use this command to see the content of stack frame of function **ah_choo()**.
...
From the above stack's content, we can see that one block above the block containing return address (0x08048d0c) is a block with address 0x55683688 and contains 0x0000000f. We'll change the value 0x08048d0c into 0x55683688 so that after function **ah_choo()** was finished in reading our 52 characters, the program will execute the instructions on a block at 0x55683688. The content of that block is our code injection!
...
See? The content of register %eax has been changed into our cookie (0x1d9ed824). This means the content of address 0x804d104 has also changed into our cookie. Let's prove it.
...
Our intuition was right, the content of address 0x804d104 has changed. How about the content of address 0x804d100?
